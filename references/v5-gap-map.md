...
| Stale memory conflict | Current memory-assembly work shows retrieval can succeed while final conflict/policy execution still fails, and explicit version metadata can support deterministic policy execution in bounded tasks | v4 handles temporal claims but not memory-record supersession or post-retrieval assembly | Require timestamp/version comparison for mutable facts; separate evidence extraction from policy execution when semantics permit | Newest is not always correct for every question type, and the reported gains are task-bounded |
...
The sources below were checked on their primary publication pages during the v5 research pass. Inclusion here does not mean every result generalizes beyond the paper's evaluated setup. `v5-research-manifest.json` is the offline canonical list used to detect title/URL drift; it does not replace live source verification.
...
14. When Do Agent Loops Mistake Stagnation for Progress? Self-Evaluation Bias and Externally Grounded Verification in Long-Running Autonomous LLM Agent Loops - arXiv:2607.25152: https://arxiv.org/abs/2607.25152
...
18. Reliable Post-Retrieval Assembly for Agent Memory: Separating Evidence Extraction from Policy Execution - arXiv:2606.01435: https://arxiv.org/abs/2606.01435
